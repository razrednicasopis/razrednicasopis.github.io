...
import { getFirestore, setDoc, doc, onSnapshot } from "https://www.gstatic.com/firebasejs/10.11.1/firebase-firestore.js";
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function checkInitialMaintenanceStatus() {
        try {
            const doc = await db.collection('settings').doc('maintenanceMode').get();
            if (doc.exists) {
                const data = doc.data();
                if (data.maintenance) {
